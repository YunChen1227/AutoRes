...
# 机器数对齐对比（弱扩展）
用户常希望"对齐卡数/机器数"做公平对比：不同配置实际占用的规模不同（由 tp/pp/dp
及 sglang 的 dp_attention 决定），直接比吞吐并不公平。`submit_query_spec` 的
`normalize_gpu_scale` 默认开启，统一按**机器数**归一：
机器数 = 总卡数 ÷ 该显卡型号的每机卡数，**允许小数**（H20 4 卡 = 0.5 机、2 卡 = 0.25 机）；
归一到组内最大机器数，吞吐类×机器数比例、concurrency 同比对齐、延迟类保持原值；
机器数相同则无操作。汇报时用"N机"口径描述规模（如 0.5机 / 2机）。
